...
export const PH_FORM_FOLDABLE_BASE_STATE: PHFormFoldableBaseStateType = { ...PH_FOLDABLE_BASE_STATE, ...{} as PHFormFoldableBaseStateType };
...
> {
  ...
  handleInputChange: (valueName: string) => React.ChangeEventHandler<HTMLInputElement & HTMLSelectElement> = (valueName: string) => {
    return (e) => this.setState(
      state => ({ ...state, [valueName]: (e.currentTarget.type === 'checkbox') ? e.currentTarget.checked : e.currentTarget.value })
    );
  }
  ...
  onReset: React.MouseEventHandler<HTMLButtonElement> = (event) => {
    ...
    if (confirm('내용을 정말 초기화하실건가요? 이 작업은 되돌릴 수 없습니다!'))
  ...
  getChildren: () => React.ReactNode = () => {
    ...
    const resetBtn = <PHButton disabled={this.state.isProcessing} buttonType='submit' variant='danger' onClick={this.onReset}>초기화</PHButton>
  ...
  render() { return <Form ref={this.formRef}>{super._render()}</Form>; }
}
