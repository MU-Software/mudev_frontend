...
const getAPIErrorDbgMsg = (apiResult: NType.APIResultType) =>
...
class API {
  ...
  #fetch: NType.APIRequestFetcherType = (method, route, accessTokenRequired, additionalHeaders, data) => {
    const fetchOption: RequestInit = objUtil.filterRecord({
      ...
      body:
        method == NType.HttpMethodType.POST ||
        method == NType.HttpMethodType.PATCH ||
        method == NType.HttpMethodType.PUT
          ? JSON.stringify(data ?? {})
          : undefined,
  ...
  #checkResponse: NType.APIResponseHandlerType = async (
    method,
    route,
    accessTokenRequired,
    additionalHeaders,
    ...
    if (!response) {
      ...
      // SUCCESS // this returns response.json()
      ...
      } else if (API.RETURNABLE_ERROR.includes(response.status)) {
        // this "possibly" returns response.json(), See RETURNABLE_ERROR for more details.
        return {
          response: response,
          header: response.headers,
          body: (await response.json()) as Record<string, unknown>,
  ...
  #apiRequest: NType.APIRequestCallableType = async (
    method,
    route,
    accessTokenRequired,
    additionalHeaders,
    ...
    if (NType.HttpMethod.indexOf(method) == -1)
      throw new NType.APIError(
        APIErrorMessage.DEFAULT_API_ERROR,
        `Requested method is not allowed: ${method}`,
        -1,
        false,
        undefined,
        undefined,
    ...
    const fetchResult: Response = await this.#fetch(
      method,
      route,
      accessTokenRequired,
      additionalHeaders,
    ...
    const fetchCheckResult = await this.#checkResponse(
      method,
      route,
      accessTokenRequired,
      additionalHeaders,
  ...
  get<T extends NType.APIResultGenericTypes = NType.APIResultGenericTypes>(
    route: string,
  ...
  post<T extends NType.APIResultGenericTypes = NType.APIResultGenericTypes>(
    route: string,
  ...
  put<T extends NType.APIResultGenericTypes = NType.APIResultGenericTypes>(
    route: string,
    data: Record<string, unknown> = {},
    accessTokenRequired = false,
    additionalHeaders = {}
  ...
  patch<T extends NType.APIResultGenericTypes = NType.APIResultGenericTypes>(
    roue: string,
    data: Record<string, unknown> = {},
    accessTokenRequired = false,
    additionalHeaders = {}
  ...
  changePassword = async (original_password: string, new_password: string, new_password_check: string) => {
    const apiResult = await this.post('account/change-password', {
      original_password,
      new_password,
      new_password_check,
    })
