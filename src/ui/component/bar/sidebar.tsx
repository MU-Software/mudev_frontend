...
const SidebarItem: React.FC<{ data: SidebarItemData }> = ({ data }) => {
  ...
  return (
    <OverlayTrigger
      ...
      overlay={(titleProps) => (
        <Tooltip {...titleProps}>
          <div style={{ width: 'max-content' }}>{data.sideBarTooltipText}</div>
        </Tooltip>
      )}
    >
      <Nav.Item
        className={navItemStyle}
        style={{ flex: 'unset', marginTop: data.shouldBePositionedOnBottom ? 'auto' : undefined }}
      >
        <Nav.Link className={sidebarIconPillsStyle} eventKey={data.route} onClick={() => navigate(data.route)}>
...
const SidebarContainerPuller: React.FC<{ sidebarContainer: React.RefObject<HTMLDivElement> }> = ({
  sidebarContainer,
}) => {
  ...
  return (
    <div className={sidebarContainerPullerStyle} onClick={onSidebarPullerClick}>
      <div id="sidebarContainerText" ref={pullerIcon}>
        ▶
      </div>
    </div>
  )
...
const Sidebar: React.FC<{ sidebarItems: SidebarItemData[] }> = ({ sidebarItems }) => {
  ...
  return (
    <div className={sidebarStyle} ref={containerRef}>
      <SidebarContainerPuller sidebarContainer={containerRef} />
      <Nav
        fill
        variant="pills"
        activeKey={item.route}
        style={{ height: '100%', flexDirection: 'column', justifyContent: 'flex-start' }}
      >
        {sidebarItems.map((v) => (
          <SidebarItem data={v} key={`sidebar-item-${v.id}`} />
        ))}
      </Nav>
    </div>
  )
