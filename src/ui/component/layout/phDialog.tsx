...
export const PHDialog: React.FC<PHDialogPropType> = (props) => {
  ...
  const closeBtn = (
    <PHButton variant="link" onClick={props.closeDialog} style={PHDialogCancelBtnInlineStyle}>
      <Icon className={PHDialogCancelBtnStyle} path={mdiClose} size="16pt" />
    </PHButton>
  )
  const actionBtns = props.actions.map((action, index) => (
    <PHButton key={index} onClick={action.onClick} variant={action.variant}>
      {action.children}
    </PHButton>
  ))

  return (
    <dialog className={PHDialogStyle} ref={dialogRef}>
      {/* Header */}
      ...
      {/* Children */}
      <div className={PHDialogChildrenStyle}>{props.children}</div>
